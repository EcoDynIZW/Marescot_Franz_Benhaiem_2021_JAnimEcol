...
output <- 
  readr::read_csv(
    here::here("data", "YBC_Birth_Pulse experiment_birth_pulse-table.csv"), 
    skip = 6
  ) %>% 
  janitor::clean_names() %>% 
  mutate(
...
gridim[which(gridim$`Between-group contact` == 1), "Between-group contact"]<-"Baseline scenario" 
```
...
ggplot(gridim, 
  ...
  scale_color_manual(values = c("#7d408a", "#eb6f6c"), 
                     breaks = c("Communal nursery", "Baseline scenario")) +
...
ggsave(here::here("figures", "fig_S1.pdf"), width = 8, height = 5, device = cairo_pdf)
```
