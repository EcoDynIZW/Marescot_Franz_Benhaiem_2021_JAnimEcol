...
[
 ...
 protected ; proportion of susceptible in the population that are protected
 infecub
]
...
to setup ; initialization-------------------------------------------------------------------
  ...
  [
    ...
    [
      set age random-exponential (age-repro)  ; randomly assign an age to individuals from 0 to 10 years old
...
ifelse frequency_transmission = true ; infection process under frequency dependent transmission
  ...
  [ ; otherwise infection process under density dependent transmission
    set infectivity rnot / (meanIL * (within-group.contact * (group-size - 1) + between-group.contact * (carrying-capacity - group-size) * (1 - young)))
  ]
...
to getimmunity
 ...
 ask patches with [clan-territory = true and count hyenas-here with [age >= age-repro] =  count hyenas-here with [age >= age-repro and disease = "recovered"]]
 [
   set sealed sealed + 1 ; counting the number of sealed group
   if count hyenas-here with [age < age-repro] > 0
   [

     set immefficiency immefficiency + count hyenas-here with [age < age-repro and disease = "susceptible"] / count hyenas-here with [age < age-repro]
     set locked locked + count hyenas-here with [age < age-repro and disease != "susceptible"] / count hyenas-here with [age < age-repro]

     set infecub infecub + count hyenas-here with [age < age-repro and disease = "infected"] ; counting the number of infected cubs in sealed groups
   ]
 ]


 if count hyenas with [age < age-repro and disease = "infected"] > 0
 [
    set infecub infecub / count hyenas with [age < age-repro and disease = "infected"] ; proportion of infected cubs that are in sealed groups
 ]
 ...
 [
   ...
   [
     ...
     set nonprot nonprot + count hyenas-here with [age < age-repro and disease = "susceptible"]  ; proprotion of susceptibles juveniles that are not protected
   ]
 ]


 if count hyenas with [age < age-repro and disease = "susceptible"] > 0
 [
   set nonprot nonprot / count hyenas with [age < age-repro and disease = "susceptible"] ; proportion of susceptible juveniles that are not protected
 ...
 set protected immefficiency * sealed
...
@#$#@#$#@
<experiments>
  <experiment name="full_simulation_cont_breeding" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="520"/>
  ...
  <experiment name="single_run_fig4_and_5" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="520"/>
    <metric>abundance</metric>
    ...
    <metric>nonprot</metric>
    <metric>locked</metric>
    <metric>count hyenas with [disease = "infected"]</metric>
    <metric>count hyenas with [disease = "infected" AND age &gt; age-repro]</metric>
    <metric>count hyenas with [disease = "infected" AND age &lt;= age-repro]</metric>
    <metric>count hyenas with [disease = "recovered" AND age &gt; age-repro]</metric>
    <metric>count hyenas with [disease = "recovered" AND age &lt;= age-repro]</metric>
    <metric>count hyenas with [disease = "susceptible" AND age &gt; age-repro]</metric>
    <metric>count hyenas with [disease = "susceptible" AND age &lt;= age-repro]</metric>
    <enumeratedValueSet variable="within-group.contact">
      <value value="0.1"/>
    </enumeratedValueSet>
    ...
    <enumeratedValueSet variable="age-repro">
      <value value="104"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="virulence">
      <value value="0.1"/>
