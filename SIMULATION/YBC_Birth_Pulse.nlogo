...
to reproduce
 ...
 if tickyear < 52
 [
    set breedingadult count patches with [clan-territory = true and count hyenas-here <=  group-size]
    set breedingadultoutput count turtles-on patches with [clan-territory = true and count hyenas-here <=  group-size]
 ...
 [
       set cubyear cub
       set cubpermonth round(cub / 12)
       ;set cubyear cubyear - cubpermonth
       set counter cubpermonth
       ask patches with [clan-territory = true and count hyenas-here <=  group-size]
       [
        ask n-of (count hyenas-here with [age >= age-repro] / 2) hyenas-here with [age >= age-repro]
        [

        if counter > 0 and cuboutput > 0
        [
...
@#$#@#$#@
<experiments>
  <experiment name="experiment_birth_pulse" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="520"/>
